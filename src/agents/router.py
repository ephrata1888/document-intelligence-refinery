    ...
    def route_and_extract(self, pdf_path: str) -> NormalizedDocument:
        ...
        for i in range(start_idx, len(strategies)):
            ...
            try:
                ...
                self._log_attempt(pdf_path, strategy, result.confidence_score, result.processing_time, result.estimated_cost)
                return result
            except LowConfidenceError as e:
                logger.info(f"Escalating {Path(pdf_path).name} from Strategy {strategy} to {strategies[i+1] if i+1 < len(strategies) else 'none'} due to {str(e)}")
                self._log_attempt(pdf_path, strategy, 0.0, 0.0, 0.0)  # Failed attempt
                if i == len(strategies) - 1:
                    raise  # C failed, but shouldn't
                continue

    def _log_attempt(self, pdf_path: str, strategy: str, confidence: float, time_taken: float, cost: float):
        entry = {
